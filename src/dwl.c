...
static const Key keys[] = {
    ...
    {MODKEY, XKB_KEY_p, spawn, {.v = (const void*)menucmd}},
    {MODKEY | WLR_MODIFIER_SHIFT, XKB_KEY_Return, spawn, {.v = (const void*)termcmd}},
...
void applybounds(Client* c, struct wlr_box* bbox) {
    ...
    c->geom.width  = MAX(1 + 2 * c->bw, c->geom.width);
    c->geom.height = MAX(1 + 2 * c->bw, c->geom.height);
...
KeyboardGroup* createkeyboardgroup(void) {
    ...
    if (!(keymap = xkb_keymap_new_from_names(context, &xkb_rules, XKB_KEYMAP_COMPILE_NO_FLAGS))) die("failed to compile keymap");
...
Monitor* dirtomon(enum wlr_direction dir) {
    ...
    if ((next = wlr_output_layout_adjacent_output(output_layout, dir, selmon->wlr_output, selmon->m.x, selmon->m.y))) return next->data;
    if ((next =
             wlr_output_layout_farthest_output(output_layout, dir ^ (WLR_DIRECTION_LEFT | WLR_DIRECTION_RIGHT), selmon->wlr_output, selmon->m.x, selmon->m.y)))
        return next->data;
    return selmon;
}
...
void motionnotify(uint32_t time, struct wlr_input_device* device, double dx, double dy, double dx_unaccel, double dy_unaccel) {
    ...
    if (cursor_mode == CurMove) {
        ...
        resize(grabc,
               (struct wlr_box){
                   .x = grabc->geom.x, .y = grabc->geom.y, .width = (int)round(cursor->x) - grabc->geom.x, .height = (int)round(cursor->y) - grabc->geom.y},
               1);
        return;
    }
...
void pointerfocus(Client* c, struct wlr_surface* surface, double sx, double sy, uint32_t time) {
    ...
    if (surface != seat->pointer_state.focused_surface && sloppyfocus && time && c && !client_is_unmanaged(c)) focusclient(c, 0);
...
void tile(Monitor* m) {
    ...
    if (n > m->nmaster)
        mw = m->nmaster ? (int)roundf((float)m->w.width * m->mfact) : 0;
    else
        mw = m->w.width;
    i = my = ty = 0;
    wl_list_for_each(c, &clients, link) {
...
void updatemons(struct wl_listener* listener, void* data) {
    ...
    wl_list_for_each(m, &mons, link) {
        if (m->wlr_output->enabled && !wlr_output_layout_get(output_layout, m->wlr_output)) wlr_output_layout_add_auto(output_layout, m->wlr_output);
    }
...
void sethints(struct wl_listener* listener, void* data) {
    ...
    if (c->isurgent && surface && surface->mapped) client_set_border_color(c, urgentcolor);
}
...
void xwaylandready(struct wl_listener* listener, void* data) {
    ...
    if ((xcursor = wlr_xcursor_manager_get_xcursor(cursor_mgr, "default", 1)))
        wlr_xwayland_set_cursor(xwayland, xcursor->images[0]->buffer, xcursor->images[0]->width * 4, xcursor->images[0]->width, xcursor->images[0]->height,
                                (int)xcursor->images[0]->hotspot_x, (int)xcursor->images[0]->hotspot_y);
}
#endif
