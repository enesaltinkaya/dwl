...
function convertBlendFile() {
  ...
  if [ $convert == 1 ]; then
    ...
    dir="data/pak_1/models/terrain/"
    mkdir -p $dir
    echo "#############################################"
    echo -n "blend to glb ${filenameWithoutExtension}... "

    # output="$(
    blender $1 --background --python scripts/0-blender-terrain.py -- $dir/$filenameWithoutExtension.glb $3
    # )"
    # if [[ $? -ne 0 ]] ; then
        # echo "$output"
        # exit 1
    ...
    echo -n "gltfpack... "
    # -tc -tq 6 -tj 32 -tu
    # export KTX_GEN_MIPMAP=0
    # note: with the meshoptimizer mipmap disable fix and basisu multithreading fix, -tq 10 is viable, fast and almost lossless
    # -tl 1024
    export KTX_GEN_MIPMAP=0
    #  -slb -si 0.8
    $GLTFPACK -vpf -cc -kn -ke -tc -tj 32 -i "$dir/${filenameWithoutExtension}.glb" -o "$dir/${filenameWithoutExtension}".glb
    echo "`du -sh "$dir/${filenameWithoutExtension}".glb | cut -f1`"
    
    mv "$dir/${filenameWithoutExtension}.glb" "$dir/${filenameWithoutExtension}"

    echo -n "zstd... "
    output="$(zstd -q -19 --rm -f "$dir/${filenameWithoutExtension}")"
    if [[ $? -ne 0 ]] ; then
        echo "$output"
        exit 1
    fi
...
mkdir -p scripts/.tmp
convertBlendFile /home/enes/project/ales/assets/Scenes/Terrain/azgaar.blend
convertBlendFile /home/enes/project/ales/assets/Scenes/Terrain/8k.blend
convertBlendFile /home/enes/project/ales/assets/Scenes/Terrain/8k-second.blend
convertBlendFile /home/enes/project/ales/assets/Scenes/Terrain/8k-third.blend
